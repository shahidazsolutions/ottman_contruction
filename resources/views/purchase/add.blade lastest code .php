                    ...
                    <div class="card">
                        ...
                        <div class="card-body admin-form">
                            ...
                            <form class="row gx-3" method="GET" action="{{route('purchase.add')}}" enctype="multipart/form-data">
                                ...
                                <div class="form-group col-sm-4">
                                    ...
                                    <select class="form-select" name="requistion_id[]"  id="" value="old('requistion_id.0')">
                                        ...
                                        @foreach ($reqs->groupBy('supplier_id')->get() as $data)
                                            @php
                                            $suppliers = \App\Models\Supplier::whereId($data->supplier_id)->first();
                                            @endphp
                                            @isset($suppliers)
                                            <option value="{{$suppliers->id}}"  @selected(old('requistion_id.0') == $suppliers->id) >{{$suppliers->name}} {{$suppliers->fname}}</option>
                                            @endif
                                        @endforeach
                                    </select>
                                    @error('requistion_id')
                            ...
                            <form class="row gx-3" method="POST" action="{{route('purchase.insert')}}" enctype="multipart/form-data">
                                ...
                                <div class="form-group col-sm-4">
                                    ...
                                    <select class="form-select" name="supplier_id[]"  id="">
                                        ...
                                        @foreach ($supplier as $data)
                                            <option value="{{$data->id}}"  @selected($purchase_requistion->supplier_id == $data->id) >{{$data->name}}</option>
                                        @endforeach
                                    </select>
                                    @error('supplier_id')
                                        <span class="text-danger">The Supplier name field is required.</span>
                                ...
                                <div class="card">
                                    <div class="card-header pb-0 d-flex align-items-cetner">
                                        <h5 class="d-flex align-items-center">Requistion Items</h5>
                                         {{--  --}}
                                        <button
                                            onclick="window.location='{{route('purchase-requistion.edit', $purchase_requistion->id ,)}}/2y10157tj6zKdfsvWi3mNuOUeBaQg55796upwcOxy7bgdYsBam5nH19q'"
                                            type="button" class="btn btn-pill btn-gradient color-4 ms-3 px-3">
                                            Update Requistion Items
                                        </button>
                                    </div>
                                    <div class="card-body admin-form" id="item">
                                ...
                                <div class="form-btn col-sm-12">
                                    <button id="success" type="sumbit"  class="btn btn-pill btn-gradient color-4">Approve</button>
                                </div>
                            </form>
                            @endisset
                        </div>
