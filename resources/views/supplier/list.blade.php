...
@section('content')
        <div class="page-body">
                                        ...
                                        <tbody>
                                            @foreach ($supplier as $key=> $data)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{ $data->email }}</td>
                                                    <td>{{ $data->address }}</td>
                                                    <th>{{ $data->balance($data->id)}}</th>
                                                    <td>
                                                        <!--                                                         <a class="btn btn-sm btn-dashed btn-pill color-6" href="{{ route('supplier.show',$data->id) }}">-->
                                                        <!--    <i class="fa-solid fa-eye"></i>-->
                                                        <!--</a>-->

                                                        <a class="btn btn-sm btn-dashed btn-pill color-6" href="{{ route('supplier.edit',$data->id) }}">
                                                ...
                                                <div class="modal fade" id="delete{{ $data->id }}" tabindex="-1"
                                                    ...
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <form action="{{route('supplier.delete',$data->id)}}">
            ...
            <!-- Container-fluid end -->
            <script>
                $(document).on("click",'.explore_supplier', function(e){
                    e.preventDefault();
                    let url = "{{route('supplier.detail', ':id' )}}";
                    url = url.replace(':id', $(this).attr('supplier_id'));
                    $.ajax({
                        url: url,
                        method: 'GET',
